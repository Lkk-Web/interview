...
核心规则：`左前缀原则`,`等值条件`字段应放在前，范围和排序字段放在后；`高并发查询`应优先使用复合索引而非索引合并，分页和排序应与索引顺序保持一致以`避免内存排序`。

```js
{ a: 1, b: -1, c: 1 } // 不是“同时命中”，而是有顺序的。
...
```js
db.orders.find({ userId: 'U1' }).sort({ createdAt: -1 }) // 内存排序（慢 + 可能报错）

.skip(100000).limit(10) ❌ 坏分页
```

- 进索引

```js
{ userId: 1, createdAt: -1 } // 直接走索引顺序扫描

db.orders.find({
  userId: 'U1',
  createdAt: { $lt: lastCreatedAt }
})
.sort({ createdAt: -1 }) //游标分页
.limit(10)
```
...
db.sessions.createIndex(
  { createdAt: 1 },
  { expireAfterSeconds: 60 * 60 * 1 } //1 小时后自动删除
)
```
- 典型应用场景
...
> 文本索引 = 为“自然语言搜索”准备的索引，不是模糊查询替代品 != `%like%`
...
👉 中文项目通常不用原生 text 索引, 会用 `ElasticSearch` / OpenSearch
