...
1. 硬链接 + 全局内容寻址存储
   - npm / yarn 安装依赖时，会把每个项目需要的包都完整复制一份到 node_modules。
   - pnpm 会把依赖先存到一个 全局存储（content-addressable store） 里，每个包只下载一次，后面项目用时直接用 硬链接/符号链接 指过去。

< 节省磁盘空间，安装速度更快。

2. 严格的 node_modules 结构（非平铺）

   - npm / yarn 会把依赖扁平化放到 node_modules，可能出现「幽灵依赖」的问题（没写在 package.json 的包也能用）。
   - pnpm 用符号链接的方式，保证 node_modules 结构和 package.json 一致，依赖关系清晰，不会乱。

3. 并行安装 + 更高效的缓存
   - pnpm 在安装依赖时会并发执行网络请求和解压，速度明显优于 npm。
   - 它的缓存机制比 npm 更高效，复用率更高。

### 9.2 npx

    •	npx = npm 5.2+ 之后附带的一个命令执行工具。
    •	作用：帮你执行项目中或远程的 npm 包里的可执行命令，不需要全局安装，如果本地没安装，会临时下载一个版本来执行，然后用完就丢弃。

| 特点           | npm                               | npx                         |
| -------------- | --------------------------------- | --------------------------- |
| 核心功能       | 包管理器（安装/卸载/依赖管理）    | 包执行器（运行 CLI 命令）   |
| 典型场景       | npm install react                 | npx create-react-app my-app |
| 全局安装依赖？ | 常常需要 (npm install -g xxx)     | 不需要，临时下载也能跑      |
| 项目命令运行   | 需要写 "scripts" 然后 npm run xxx | 直接执行命令，不用写脚本    |
